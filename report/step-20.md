...
- 실패한 요청을 캐싱하거나 대체 응답을 제공하여 사용자 경험을 개선

#### 2) 적용할 API 및 적용 방법

| 적용 대상 API       | 서킷 브레이커 적용 방식                     |
|-----------------|-----------------------------------|
| 토큰 발급 API       | Redis 캐싱을 활용한 재시도 전략 적용           |
| 좌석 예약 API       | 요청이 실패하면 일정 시간 동안 동일한 좌석 예약 요청 차단 |
| 예약 가능 시간 조회 API | 일정 요청 실패율 이상 발생 시 캐싱된 응답 제공       |
